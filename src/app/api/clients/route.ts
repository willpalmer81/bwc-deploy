...
export async function GET() {
  ...
  const clients = await sql`
    SELECT c.*, a.name as arc_name, p.name as contact_name,
    ...
    LEFT JOIN people p ON c.contact_id = p.id
    ORDER BY c.name
  `;
...
export async function POST(request: Request) {
  ...
  const result = await sql`
    INSERT INTO clients (name, arc_id, routing_mode, contact_id, notes)
    VALUES (${name}, ${arc_id || null}, ${routing_mode}, ${contact_id || null}, ${notes || null})
    RETURNING *
  `;
